...
#define MAX_TRIES 3000 // --> not applicable for the ECS 36B project
...
// variables defined by the student
char sol[WORD_LENGTH] = "_____";
...
// function made by studetn to compare the guess with a string sol that contains characters in known position of the solution.
bool checkSol(char gs[WORD_LENGTH]);
...
/*
compares the guess to the discernable part of the solution
*/
bool checkSol(char gs[WORD_LENGTH]) {
...
void gameLoop()
{
  ...
  int guesses = 0;
  /* this variable is not useful  as we are not asking for user input anymore. */
  int scan = 0;
  puts(
...
#define Felix_HACK
      ...
      if (Felix_Hack == 1)
	{
      ...
      /*
      Instead of prompting the user for input, we scan SOLUTIONS.TXT using our
      filters
      */
      if ((scan = scanf("%5s", guess)) == 1 && strlen(guess) == WORD_LENGTH)
      // if ((bc = fscanf(fpSol, "%s", guess)) != EOF && strlen(guess) == WORD_LENGTH)
	{

    while(!checkSol(guess) && fscanf(fpSol, "%s", guess) != EOF) {
      continue;
    }
    ...
    /* We remove this because the scan variable is not useful anymore*/
      else
	{
	  if (scan == EOF)
	    {
